import consts
from comp_window import CompWindow
...
GRAPH_LOCATION = (0, 0)
PB_PRINT_ELLIPSE_LOCATION = (7, 0)
PB_PRINT_CIRCLE_LOCATION = (8, 0)
...
class Main(QMainWindow):
    ...
    def ui_buttons(self) -> None:
        self.pb_print_ellipse = QPushButton(
            "Построить спектр концентрических окружностей"
        )
        self.pb_print_circle = QPushButton("Построить эллипс")
        self.pb_edit_background = QPushButton("Изменить фон")
        self.pb_reset = QPushButton("Очистить экран")
        for pb in (
            self.pb_print_ellipse,
            self.pb_print_circle,
            self.pb_reset,
    ...
    def ui_layout(self) -> None:
        ...
        self.layout.addWidget(self.pb_print_ellipse, *PB_PRINT_ELLIPSE_LOCATION)
        self.layout.addWidget(self.pb_print_circle, *PB_PRINT_CIRCLE_LOCATION)
    ...
    def connecter(self) -> None:
        ...
        self.pb_print_ellipse.clicked.connect(self.open_spectrum_window)
        self.pb_print_circle.clicked.connect(self.open_line_window)
    ...
    def show_task(self) -> None:
        ...
        self.msg_box.setText(
            "Реализация и исследование алгоритмов построения окружности и эллипса"
        )
        self.msg_box.show()
    ...
    def show_instruction(self) -> None:
        ...
        self.msg_box.setText(
            "Для построения эллипса (окружности) необходимо заполнить в форме все поля"
            + " (все поля задаются целыми числами) "
            + 'и нажать на кнопку "Построить".'
        )
        self.msg_box.show()
