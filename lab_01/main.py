...
def draw_point(graph: pg.PlotWidget, table: QTableWidget, point: QPointF) -> None:
    graph.clear()
...
class InputPoint(QWidget):
    def __init__(self: QWidget, tw: QTableWidget, graph: pg.PlotWidget, start_point: QPointF) -> None:
        super().__init__()
        self.tw = tw
        self.graph = graph
...
class Main(QMainWindow):
    ...
    def graph_clicked(self: QMainWindow, ev) -> None:
        if ev.button() == Qt.MouseButton.RightButton:
            ...
            self.last_added_point_by_click = pg.Point(pos.x(), pos.y())
            if added_point in get_points_list(self.tw):
                self.msgBox.setWindowTitle("Инфо")
                self.msgBox.setText("Данная точка была добавлена в множество ранее")
                self.msgBox.show()
    ...
    def print_solve(self: QMainWindow) -> None:
        ...
        if self.solve_created:
            ...
            self.msgBox.show()
            return
        ans = calc.get_solve(get_points_list(self.tw))
        if ans is None:
            self.msgBox.setWindowTitle("Инфо")
