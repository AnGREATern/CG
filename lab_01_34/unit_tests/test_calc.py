...
sys.path.append("./src")
from geom import find_max_angle, orthocenter, projection
...
class TestSolve(unittest.TestCase):
    ...
    def test_second_quarter(self):
        ans = find_max_angle(
            [Point(-1.0, 3.0), Point(-4.0, 7.0), Point(-8.0, 1.0), Point(-5.0, 5.0)]
        )
    ...
    def test_forth_quarter(self):
        ans = find_max_angle(
            [Point(3.0, -4.0), Point(5.0, -11.0), Point(20.0, -10.0), Point(1.0, -1.0)]
        )
        self.assertAlmostEqual(ans[0], 76.06220279174576)

    def test_orthocenter_search(self):
        a = Point(-1, 0)
        b = Point(0.5, 0.86602540378)
        c = Point(0.5, -0.86602540378)
        ans = orthocenter(a, b, c)
        self.assertAlmostEqual(ans.x(), 0)
        self.assertAlmostEqual(ans.y(), 0)

    def test_projection_search(self):
        a = Point(1, 2)
        b = Point(10, 0)
        ans = projection(a, b)
        self.assertAlmostEqual(ans.x(), 1)
        self.assertAlmostEqual(ans.y(), 0)
